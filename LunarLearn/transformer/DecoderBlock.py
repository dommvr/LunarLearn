...
class DecoderBlock(BaseLayer):
    def __init__(self,
        ...
        self.self_attn = MultiHeadAttention(d_model, n_heads, attention, att1_keep_prob)
        self.cross_attn = MultiHeadAttention(d_model, n_heads, attention, att2_keep_prob)
    ...
    def forward(self, x: Tensor, mask=None, context=None) -> Tensor:
        if self.norm_position == "pre":
            attn1_out = self.self_attn(self.norm1(x), mask=mask)
            x = x + attn1_out

            if self.cross_attn:
                attn2_out = self.cross_attn(self.norm2(x), context=context)
                x = x + attn2_out
