...
class BatchNorm2D(BaseLayer):
    ...
    """
    def __init__(self, momentum=0.9, epsilon=1e-3):
    ...
    def initialize(self, input_shape):
        ...
        W = xp.ones((1, n_C_prev, 1, 1), dtype=DTYPE)
        b = xp.zeros((1, n_C_prev, 1, 1), dtype=DTYPE)

        self.W = Parameter(W, requires_grad=True)
        self.b = Parameter(b, requires_grad=True)

        self.running_mean = xp.zeros((1, n_C_prev, 1, 1), dtype=DTYPE)
        self.running_var = xp.ones((1, n_C_prev, 1, 1), dtype=DTYPE)
    ...
    def forward(self, Z: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            mean = ops.mean(Z, axis=(0, 2, 3), keepdims=True)
            var = ops.var(Z, axis=(0, 2, 3), keepdims=True)
